...
/** Head of chain for one network, served at the root of that network's host. */
export async function Overview({ net }: { net: NetworkId }) {
    ...
    <>
      ...
      <div className="grid-2">
        <Panel
          ...
          live
          action={
            <Link className="panel-action" href="/blocks">
              View all →
            </Link>
          }
        >
              ...
              {o.blocks.map((block) => (
                <div key={block.height.toString()} className="row row-block">
                  <span className="row-primary">
                    <Hash value={block.height.toString()} kind="block" full />
                  </span>
        ...
        <Panel
          ...
          live
          action={
            <Link className="panel-action" href="/txs">
              View all →
            </Link>
          }
        >
              ...
              {o.recent.slice(0, 12).map((tx) => (
                <div key={tx.signature} className="row row-tx">
                  <span className="row-primary" style={{ overflow: "hidden" }}>
                    <Hash value={tx.signature} kind="tx" head={10} tail={8} />
                  </span>
                  <span className="row-secondary">
